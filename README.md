# 👋 Hi, I'm Ajay Vasudevan

### B.Sc. Data Science | Data Analytics | AI & Machine Learning

I'm a Data Science student interested in **Data Analytics, Business Intelligence, Machine Learning, and Generative AI**.

I enjoy turning data into meaningful insights using **Python, SQL, Power BI, Excel, and AI technologies**.

---

## 🛠️ Skills

**Languages:** Python, SQL  
**Data:** Pandas, NumPy, MySQL, MongoDB  
**BI:** Power BI, Power Query, DAX, Excel  
**AI/ML:** Machine Learning, NLP, Transformers, Generative AI, LLM, RAG  
**Tools:** GitHub, VS Code, Google Colab

---

## 📂 Projects

- 📊 **Excel Business Insights** — Data cleaning, KPI analysis & dashboard
- 📈 **Power BI Dashboard** — Power Query, DAX, data modeling & visualization
- 🤖 **Product Reviews Sentiment Analysis** — NLP & Machine Learning
- 🧠 **Generative AI & RAG** — LLM, Prompt Engineering & RAG

---

## 📊 GitHub status

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=ajayvasudevancbe-Datascience&show_icons=true&theme=tokyonight&hide_border=true" height="160"/>
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=ajayvasudevancbe-Datascience&theme=tokyonight&hide_border=true" height="160"/>
</p>

---

## 🌐 Connect With Me

[LinkedIn](https://www.linkedin.com/in/ajay-vasudevan-8800953a1/)  
[GitHub](https://github.com/ajayvasudevancbe-Datascience)

---

### `DATA • INSIGHTS • IMPACT`







